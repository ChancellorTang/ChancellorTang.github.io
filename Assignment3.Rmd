...
``` {r}
df$death3 <- cut(df$Deaths, breaks = c(-Inf, summary(df$Deaths)[1], summary(df$Deaths)[3],Inf), 
...
``` {r}
df1 <- df[(df$Date>='2021-01-01')&(df1$State == "Rhode Island"),]
...
```{r}
df1 = df[(df$State=='Rhode Island')&(df$Date>='2021-06-01'),]
