...
``` {r}
df1 <- df %>% filter(Vaccine_Type != "All") %>% group_by(Date, Vaccine_Type) %>% summarise(vac_admin=sum(Doses_admin, na.rm=TRUE))
...
``` {r}
df2 <- df %>% filter(Province_State %in% c('Massachusetts','Connecticut', 'Rhode Island', 'Maine', 'Vermont', 'New Hampshire'), Vaccine_Type != "All") %>% group_by(Date, Vaccine_Type) %>% summarise(vac_admin=sum(Doses_admin, na.rm=TRUE))
...
df3 <- df %>% filter(Province_State %in% c('Massachusetts','Connecticut', 'Rhode Island', 'Maine', 'Vermont', 'New Hampshire'), Vaccine_Type != "All") %>% group_by(Month, Province_State) %>% summarise(vac_admin=sum(Doses_admin, na.rm=TRUE))
...
```{r}
df4 <- df %>% filter(Province_State %in% c('Massachusetts','Connecticut', 'Rhode Island', 'Maine', 'Vermont', 'New Hampshire'), Vaccine_Type != "All") %>% group_by(Date, Province_State) %>% summarise(vac_admin=sum(Doses_admin, na.rm=TRUE))
