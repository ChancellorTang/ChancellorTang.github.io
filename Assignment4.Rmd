...
# sum of the square root of x
x%>%sqrt%>%sum

# log of the square root of x
x%>%sqrt%>%log

# log base 2 of 16 (need new answer)
16 %>% log2() 
...
``` {r}
df %>% 
    filter(year(df$date)== 2021, state== "RI") %>% 
    group_by(weekend) %>% 
    summarise(cases = median(positiveIncrease))
```
...
df %>% 
  filter(weekend == 1) %>% 
  group_by(month) %>% 
  summarise(weekend_deaths = sum(death, na.rm = TRUE)) %>% 
  arrange(-weekend_deaths)
```