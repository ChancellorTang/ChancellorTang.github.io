...
``` {r}
df %>% group_by(Sex) %>% summarize(totalSurvivors = sum(Survived))
```
...
``` {r}
summary(df$Age)
df$Age1 <- cut(df$Age, breaks = c(-Inf, 22, 35,Inf), labels=c('young','middle','old'))
df %>% group_by(Age1) %>% summarize(totalSurvivors = sum(Survived))
```
...
```{r}
d = read_csv('https://bryantstats.github.io/math421/slides/car.csv')
sum(is.na(d))
d = d %>% filter(STATE == "Rhode Island")
sum(is.na(d))
head(d)
```
...
``` {r}
table(d$TRAV_SP)
d$TRAV_SP <- na_if(na_if(d$TRAV_SP, "Unknown"), "Not Rep")
d$TRAV_SP <- as.numeric(gsub("([0-9]+).*$", "\\1", d$TRAV_SP))
